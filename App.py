...
for _, row in df_raw.iterrows():
    ...
    c3.markdown('<span style="color:{}">**{}**</span>'.format(
        pnl_color, fmt_usd(pnl_val)), unsafe_allow_html=True)
    c4.write('Vol: ' + fmt_usd(vol_val))

    in_watch = wallet in st.session_state.watchlist
    btn_label = '✓ Watch' if in_watch else '+ Watch'
...
```
if not nomi2:
    st.info('Carica prima la leaderboard dal tab Leaderboard.')
else:
    ...
    if wallet2:
        ...
        st.divider()
        st.subheader('Posizioni aperte — copia un trade')
...
if not nomi3:
    ...
    if st.button('Confronta i due trader', use_container_width=True):
...
if keyword:
    ...
    if not nomi4:
                ...
                for p in get_positions(w4):
                    ...
                    if keyword.lower() in title.lower():
                        ...
                        risultati.append({
                            ...
                            'Mercato': title,
                            'Esito': p.get('outcome', 'N/A'),
                            'Prezzo attuale': '{:.0f}%'.format(float(p.get('curPrice', 0)) * 100),
        ...
        if risultati:
            ...
            st.subheader('Vai direttamente al mercato per copiare')
            for r in risultati:
                ca4, cb4 = st.columns([5, 1])
                ca4.write('**{}** — {} ({}) @ {}'.format(
                    r['Trader'], r['Mercato'][:50], r['Esito'], r['Prezzo attuale']))
